...
/**
 * NewTaskBottomSheetFragment - Form pembuatan task baru dalam popup dialog yang user-friendly.
 */
class NewTaskBottomSheetFragment : BottomSheetDialogFragment() {
    ...
    companion object {
        ...
        fun newInstance(): NewTaskBottomSheetFragment {
            return NewTaskBottomSheetFragment()
        }
    }
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_new_task_bottom_sheet, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        TaskManager.init(requireContext())

        buttonClose = view.findViewById(R.id.btn_close_new_task)
        buttonClose.setOnClickListener { dismiss() }

        editTaskTitle = view.findViewById(R.id.edit_task_title)
        spinnerCategory = view.findViewById(R.id.spinner_category)
        textViewDueDate = view.findViewById(R.id.text_due_date)
        textViewDueTime = view.findViewById(R.id.text_due_time)
        checkBoxReminder = view.findViewById(R.id.checkbox_reminder)
        textViewReminderTime = view.findViewById(R.id.text_reminder_time)
        spinnerRepeatType = view.findViewById(R.id.spinner_repeat_type)
        editNotes = view.findViewById(R.id.edit_notes)
        buttonSave = view.findViewById(R.id.btn_save_task)
        buttonCancel = view.findViewById(R.id.btn_cancel_task)
    ...
    private fun setupRepeatTypeSpinner() {
        ...
        val repeatAdapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, repeatTypes)
    ...
    private fun setupDateTimeInputs() {
        ...
        checkBoxReminder.setOnCheckedChangeListener { _, isChecked ->
            ...
            if (isChecked) {
                textViewReminderTime.visibility = View.VISIBLE
            }
        }
    ...
    private fun saveTask() {
        ...
        if (!isValidTitle) {
            ...
            UIHelper.showToast(requireContext(), titleError ?: "Invalid task title")
            return
        }
        ...
        if (selectedCategoryIndex < 0 || selectedCategoryIndex >= categories.size) {
            UIHelper.showToast(requireContext(), "Please select a category")
        ...
        if (!isValidNotes) {
            UIHelper.showToast(requireContext(), notesError ?: "Notes too long")
            return
        }
        ...
        try {
            ...
            UIHelper.showToast(requireContext(), "Task \"$title\" created successfully")
            dismiss()
        } catch (e: Exception) {
            UIHelper.showErrorDialog(requireContext(), "Error", "Failed to create task: ${e.message}")
        }
    }
    ...
    private fun formatTime(time: LocalTime): String {
        val formatter = if (DateFormat.is24HourFormat(requireContext())) {
